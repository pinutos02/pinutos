...
export function ReservationPage() {
  ...
  return (
    <div className="pt-24 md:pt-40 pb-32 px-6 md:px-12 min-h-screen bg-warm-cream">
      <div className="max-w-6xl mx-auto">
        <header className="mb-12 md:mb-20 text-center space-y-4">
          ...
          <h1 className="font-serif text-4xl md:text-6xl font-bold text-brand-stone italic leading-none">Reserve Your Spot</h1>
          <p className="text-stone-500 max-w-lg mx-auto text-sm leading-relaxed px-4">
            Planning a celebration or a family dinner? Secure your table in our heritage dining room.
          </p>
        </header>

        {!user ? (
          <div className="bg-white p-12 md:p-20 text-center border border-brand-sepia shadow-sm max-w-2xl mx-auto">
            ...
            <p className="text-stone-500 mb-10 text-sm max-w-sm mx-auto leading-relaxed px-4">We require guest verification to manage our buffet capacity and ensure the best service for everyone.</p>
            <button 
              onClick={login}
              className="w-full md:w-auto px-12 py-4 bg-brand-stone text-white text-[10px] uppercase font-black tracking-[0.3em] hover:bg-black transition-all"
            >
              Verify with Google
            </button>
          </div>
        ) : (
          <div className="bg-white overflow-hidden border border-brand-sepia shadow-sm flex flex-col md:flex-row min-h-[600px] md:min-h-[650px]">
            {/* Sidebar with Steps */}
            <div className="w-full md:w-1/3 bg-brand-stone p-8 md:p-12 text-white flex flex-col justify-between">
              <div className="space-y-12">
                <div>
                   <h3 className="text-[10px] uppercase tracking-[0.4em] font-black text-brand-gold mb-8 md:mb-10">Application Progress</h3>
                   <div className="grid grid-cols-3 md:grid-cols-1 gap-4 md:gap-10">
                    {[
                      { s: 1, label: 'Guest', longLabel: 'Guest Particulars', icon: Users },
                      { s: 2, label: 'Date', longLabel: 'Temporal Details', icon: Calendar },
                      { s: 3, label: 'Special', longLabel: 'Special Provisions', icon: MessageSquare }
                    ].map((it) => (
                      <div key={it.s} className={cn(
                        "flex flex-col md:flex-row items-center md:items-center gap-2 md:gap-6 transition-all",
                        step === it.s ? "opacity-100 md:translate-x-2" : "opacity-30"
                      )}>
                        <div className={cn(
                          "w-10 h-10 md:w-12 md:h-12 flex items-center justify-center border transition-all shrink-0",
                          step === it.s ? "border-brand-gold text-brand-gold" : "border-white/20 text-white"
                        )}>
                          <it.icon className="w-4 h-4 md:w-5 md:h-5" />
                        </div>
                        <div className="text-center md:text-left">
                          <p className="hidden md:block text-[9px] uppercase tracking-[0.3em] font-black text-white/50 mb-1">Phase 0{it.s}</p>
                          <p className="font-serif text-[10px] md:text-sm font-bold tracking-wide italic leading-none md:leading-normal">
                             <span className="md:hidden">{it.label}</span>
                             <span className="hidden md:inline">{it.longLabel}</span>
                          </p>
                        </div>
                      </div>
                    ))}
              ...
              <div className="hidden md:block border border-white/10 p-8 mt-12 bg-white/5">
            ...
            {/* Form Area */}
            <div className="w-full md:w-2/3 p-8 lg:p-20 bg-white">
              <form onSubmit={handleSubmit} className="h-full flex flex-col">
                ...
                <div className="mt-auto pt-10 md:pt-20 flex flex-col sm:flex-row gap-4 md:gap-6">
                  {step > 1 && (
                    <button 
                      type="button" 
                      onClick={prevStep}
                      className="w-full sm:flex-1 py-4 border border-brand-sepia font-black text-[10px] uppercase tracking-[0.3em] text-stone-500 hover:bg-stone-50 transition-all flex items-center justify-center gap-3 order-2 sm:order-1"
                    >
                  ...
                  {step < 3 ? (
                    <button 
                      ...
                      className="w-full sm:flex-[2] py-4 bg-brand-stone text-white font-black text-[10px] uppercase tracking-[0.3em] hover:bg-black disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-3 order-1 sm:order-2"
                    >
                      Proceed
                    ...
                    <button 
                      type="submit"
                      disabled={loading}
                      className="w-full sm:flex-[2] py-4 bg-brand-gold text-white font-black text-[10px] uppercase tracking-[0.3em] hover:bg-brand-stone disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-3 order-1 sm:order-2"
                    >
